...
SELECT 0, 1100000000+g1.nn_gid, 1400000000+g2.nn_gid, 11, 14, 93, 5, 't', g1.poi_id FROM
(SELECT c.poi_id, (pgis_fn_nn(c.geom, 0.002, 1, 10, '(SELECT * FROM osm_point INNER JOIN vertices ON osm_point.osm_id=vertices.raw_point_id WHERE vertices.mode_id=11)', 'true', 'osm_id', 'way')).* FROM (SELECT * FROM park_and_rides) c) AS g1,
...
UPDATE switch_points SET switch_point_id=9300000000+id WHERE switch_point_id=0;
...
ALTER TABLE switch_points DROP CONSTRAINT switch_points_pkey;
INSERT INTO switch_points (switch_point_id, from_vertex_id, to_vertex_id, from_mode_id, to_mode_id, type_id, cost, is_available, ref_poi_id)
SELECT 0, 1100000000+g1.nn_gid, 1300000000+g2.nn_gid, 11, 13, 93, 5, 't', g1.poi_id FROM
(SELECT c.poi_id, (pgis_fn_nn(c.geom, 0.002, 1, 10, '(SELECT * FROM osm_point INNER JOIN vertices ON osm_point.osm_id=vertices.raw_point_id WHERE vertices.mode_id=11)', 'true', 'osm_id', 'way')).* FROM (SELECT * FROM park_and_rides) c) AS g1,
